...
jQuery(function($){
    window.um_add_plugin_icons = function(info){
        $('#the-list tr[data-plugin]').each(function(k,v){
            ...
            if(!info['filters']['disable_auto_updates'][slug] && info['auto_update_allowed']){
                is_auto_update = true;
            }
            if(info['updates_disabled'] || info['filters']['disable_updates'][slug]){
            ...
            $(v).find('.check-column').append('<span class="dashicons dashicons-update um-plugin-status ' + update_class + '"></span>');
